...
static char e_strtab2[STR_SIZE];
...
void init_elf(const char* elf_file) {
  ...
  init_elf2("/home/trace/trace/learning/ysyx/ysyx-workbench/navy-apps/tests/file-test/build/file-test-riscv32");
  return;
...
void init_elf2(const char* elf_file) {
  if (!elf_file) return;
  Elf32_Ehdr e_hdr;
  fp = fopen(elf_file, "rb");
  Check(fp != NULL, "open %s failed", elf_file);
  ...
  size = fread(e_strtab2, 1, e_sections[str_idx].sh_size, fp);
  Check(size == e_sections[str_idx].sh_size, "expected read %u, but %lu", e_sections[str_idx].sh_size, size);

  /* read symbol table to symbols array */
  uint32_t sh_size = e_sections[sym_idx].sh_size;
  uint32_t sh_entsize = e_sections[sym_idx].sh_entsize;
  e_symnum2 = sh_size / sh_entsize;
  // 如果 symbol 数量大于 数组长度，那就不用读了。
  Check(e_symnum2 <= SYM_NUM, "symbol array overflow!");

  rewind(fp);
  fseek(fp, e_sections[sym_idx].sh_offset, SEEK_SET);
  size = fread(e_symbols2, sh_entsize, e_symnum2, fp);
  Check(size == e_symnum2, "expected read %u, but %lu", e_symnum2, size);
